_base_ = []
custom_imports = dict(imports=['projects.bevformer', 'projects.unitopo'])
...
lanes_group = (3, 3) # lane, connected lane
...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=1.0 * 1.2),
        loss_bbox=dict(type='L1Loss', loss_weight=2.5 * 1.2),
        loss_iou=dict(type='GIoULoss', loss_weight=1.0 * 1.2),
        test_cfg=dict(max_per_img=100)),
    lane_head=dict(
        type='UniTopoHead',
        num_classes=1,
        in_channels=_dim_,
        num_query=num_lanes_one2one * (lanes_group[0] + lanes_group[1]),
        num_lanes_one2one=num_lanes_one2one,
        lanes_group=lanes_group,
        bev_h=bev_h_,
        bev_w=bev_w_,
        pc_range=point_cloud_range,
        pts_dim=pts_dim,
        sync_cls_avg_factor=False,
        reg_embed_dims=_dim_ * 2,
        code_size=code_size,
        code_weights= [1.0 for i in range(code_size)],
        transformer=dict(
            type='UniTopoTransformer',
            embed_dims=_dim_,
            pts_dim=pts_dim,
            decoder=dict(
                type='UniTopoDecoder',
    ...
    train_cfg=dict(
        bbox=dict(
            assigner=dict(
                ...
                cls_cost=dict(type='FocalLossCost', weight=1.0 * 1.2),
                reg_cost=dict(type='BBoxL1Cost', weight=2.5 * 1.2, box_format='xywh'),
                iou_cost=dict(type='IoUCost', iou_mode='giou', weight=1.0 * 1.2))),
        lane=dict(
            assigner=dict(
...
lr_config = dict(
    ...
    min_lr_ratio=1e-3)
total_epochs = 48
evaluation = dict(interval=48, pipeline=test_pipeline)
