...
GT_COLOR = (0, 0, 255)
PRED_COLOR = (0, 0, 255)
GT_COLOR_TOPOLOGY = (0, 0, 127)
PRED_COLOR_TOPOLOGY = (0, 0, 127)
            ...
            for l1_idx, lclc in enumerate(gt_lclc):
                for l2_idx, connected in enumerate(lclc):
                    if connected:
                        ...
                        image = cv2.arrowedLine(image, (p1[1], p1[0]), (p2[1], p2[0]), GT_COLOR_TOPOLOGY, max(round(scale * 0.1), 1), tipLength=0.03)
            ...
            for l1_idx, lclc in enumerate(pred_lclc):
                for l2_idx, connected in enumerate(lclc):
                    if connected:
                        ...
                        image = cv2.arrowedLine(image, (p1[1], p1[0]), (p2[1], p2[0]), PRED_COLOR_TOPOLOGY, max(round(scale * 0.1), 1), tipLength=0.03)
