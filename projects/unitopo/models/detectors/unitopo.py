...
class UniTopo(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        super(UniTopo, self).__init__(**kwargs)
    ...
    def forward_dummy(self, img):
        # dummy_metas = None
        # json dump dummy_metas: torch/nn/parallel/distributed.py
        # json load dummy_metas
        import json
        dummy_metas = json.load(open("dummy.json", "r"))
        dummy_metas['lidar2img'] = [np.array(arr, dtype=np.float64) for arr in dummy_metas['lidar2img']]
        dummy_metas['crop_shape'] = [np.array(arr, dtype=np.float64) for arr in dummy_metas['crop_shape']]
        dummy_metas['img_norm_cfg'] = {'mean': np.array([123.675, 116.28 , 103.53], dtype=np.float32), 
                                        'std': np.array([58.395, 57.12 , 57.375], dtype=np.float32), 
                                        'to_rgb': True}
        dummy_metas['can_bus'] = np.array(dummy_metas['can_bus'], dtype=np.float64)
        dummy_metas['lidar2global_rotation'] = np.array(dummy_metas['lidar2global_rotation'], dtype=np.float64)

        # from tqdm import tqdm
        # repetitions = 1000
        # for rep in tqdm(range(repetitions)):
        #     self.forward_test(img=img, img_metas=[dummy_metas])
    ...
    def forward_train(self,
        ...
        if self.bev_seg_head is not None:
            outputs_seg = self.bev_seg_head(bev_feats) # bs, cls, h, w
            loss_seg = self.bev_seg_head.loss(outputs_seg, gt_lane_mask)
            losses['bev_seg'] = loss_seg
    ...
    def simple_test_pts(self, x, img_metas, img=None, prev_bev=None, rescale=False):
        ...
        # breakpoint()
        try:
            outs = self.pts_bbox_head.forward_test(x, bev_feats, img_metas, te_feats, te_cls_scores)
        except:
