...
class OpenLaneV2_subset_A_Dataset(Custom3DDataset):
    ...
    def __init__(self,
                 ...
                 bev_seg=False,
                 pc_range=[-51.2, -25.6, -2.3, 51.2, 25.6, 1.7], # x, y, z
                 bev_h=100, # y
                 bev_w=200, # x
                 **kwargs):
    ...
    def get_ann_info(self, index):
        ...
        gt_lane_mask = np.zeros((1, self.bev_h, self.bev_w), dtype=np.uint8) # cls, h, w
        if self.bev_seg:
            for lane in gt_lanes:
                coor = copy.deepcopy(lane)[::20, :2]
                # scale
                coor[:, 0] = coor[:, 0] * self.bev_w / (self.pc_range[3] - self.pc_range[0]) 
                coor[:, 1] = coor[:, 1] * self.bev_h / (self.pc_range[4] - self.pc_range[1]) 
                # affine_transform
                coor[:, 0] += self.bev_w // 2
                coor[:, 1] += self.bev_h // 2
                if len(coor) >= 2:
                    cv2.polylines(gt_lane_mask[0], np.int32([coor]), isClosed=False, color=1, thickness=3)
    ...
    def format_openlanev2_gt(self):
        ...
        for idx in range(len(self.data_infos)):
            ...
            # breakpoint()
            # row_idx, col_idx = np.where(info['annotation']['topology_lclc'] == 1)
            # if len(row_idx):
            #     new_lane_centerline = []
            #     for prev_idx, next_idx in zip(row_idx, col_idx):
            #         prev_lane, next_lane = info['annotation']['lane_centerline'][prev_idx], info['annotation']['lane_centerline'][next_idx]
            #         new_lane_centerline.append(dict(points=np.concatenate((prev_lane['points'], next_lane['points'][1:]), axis=0)[::2]))
            #     info['annotation']['lane_centerline'] = new_lane_centerline
            #     info['annotation']['topology_lclc'] = info['annotation']['topology_lclc'][col_idx][:, row_idx]
            #     info['annotation']['topology_lcte'] = info['annotation']['topology_lcte'][col_idx]

            gt_dict[key] = info
        return gt_dict

    def format_openlanev2_gt_v1(self):
        gt_dict = {}
        for idx in range(len(self.data_infos)):
            info = copy.deepcopy(self.data_infos[idx])
            key = (self.split, info['segment_id'], str(info['timestamp']))
            ...
            for lane in info['annotation']['lane_centerline']:
                if len(lane['points']) == 201:
                    lane['points'] = lane['points'][::20]  # downsample points: 201 --> 11
                if lane['is_intersection_or_connector'] == False:
                    lanes.append(lane)
            info['annotation']['lane_centerline'] = lanes
            gt_dict[key] = info
        return gt_dict
    ...
    def evaluate(self, results, logger=None, show=False, out_dir=None, **kwargs):
        ...
        # gt_dict = self.format_openlanev2_gt_v1()
    ...
    def show(self, results, out_dir, score_thr=0.3, show_num=20, **kwargs):
        ...
        for idx, result in enumerate(results):
            ...
            #            '11140',
            #         ]
            # if info['segment_id'] not in que_set:
            #     continue
            # ste_set = [315969919349927217, 
            #            315974100949927217, 
            #            315974474549927223,
            #            315971340649927218,
            #            315969641849927214,
            ...
            #            1542799528412460,
            #         ]
            # if info['segment_id'] not in que_set or info['timestamp'] not in ste_set:
            #     continue
            ...
            # bev_img = show_bev_results(gt_lanes, pred_lanes, only='gt', map_size=[-52, 52, -27, 27], scale=20)
            # output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/bev_gt.jpg')
            # mmcv.imwrite(bev_img, output_path)
            # bev_img = show_bev_results(gt_lanes, pred_lanes, only='pred', map_size=[-52, 52, -27, 27], scale=20)
            ...
            # output_path_gt = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/conn_gt.jpg')
            # mmcv.imwrite(conn_img_gt, output_path_gt)
            output_path_pred = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/conn_pred.jpg')
            mmcv.imwrite(conn_img_pred, output_path_pred)
